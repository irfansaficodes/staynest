...
router.get("/forecast", verifyToken, requireHotelOwner, async (req: Request, res: Response) => {
  try {
    const forecastData = await getForecastData();
    res.status(200).json(forecastData);
  } catch (error) {
...
router.get("/system-stats", verifyToken, requireHotelOwner, async (req: Request, res: Response) => {
  try {
    const performanceData = await getSystemStatsData();
    res.status(200).json(performanceData);
  } catch (error) {
