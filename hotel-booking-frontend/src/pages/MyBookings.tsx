...
const MyBookings = () => {
        ...
        <Card className="max-w-lg w-full">
          ...
          <CardContent className="space-y-4">
            <p className="text-muted-foreground">
              To view your bookings, please sign in to your account.
            </p>
            <Link to="/sign-in">
  ...
  if (!hotels || hotels.length === 0) {
    return (
      <EmptyState
        ...
        description="You haven't made any bookings yet. Start exploring and book your next stay!"
  ...
  const handleCancelBooking = async (bookingId: string, totalCost: number, checkIn: Date) => {
    const now = new Date();
    ...
    if (hoursUntilCheckIn > 48) {
      ...
      refundPercentage = 25;
      refundPolicy = "25% refund (< 24 hours before check-in)";
    } else {
      refundPercentage = 0;
      refundPolicy = "No refund (check-in has passed)";
    }

    const refundAmount = Math.round((totalCost * refundPercentage) / 100);

    if (!window.confirm(
      `Cancel this booking?\n\n` +
      `Refund Policy: ${refundPolicy}\n` +
      `Refund Amount: ₹${refundAmount} of ₹${totalCost}\n\n` +
    ...
    try {
      ...
      showToast({
        ...
        description: `₹${refundAmount} will be refunded within 24-48 hours`,
        type: "SUCCESS",
      });
      ...
      const msg = error instanceof Error
        ? error.message
        : (error as { response?: { data?: { message?: string; errors?: { msg: string }[] } } })?.response?.data?.message
          || (error as { response?: { data?: { message?: string; errors?: { msg: string }[] } } })?.response?.data?.errors?.[0]?.msg
                          ...
                          <div className="flex gap-2">
                            ...
                            {(booking.status === "pending" || booking.status === "confirmed") && (
                              <Button
                                variant="destructive"
                                size="sm"
                                onClick={() => handleCancelBooking(booking._id, totalPrice, booking.checkIn)}
                                disabled={cancellingId === booking._id}
                                title="Free cancellation up to 24 hours before check-in"
                              >
